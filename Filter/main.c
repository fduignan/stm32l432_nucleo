...
/* This program implements a mono audio low pass filter
The filter was designed using the following octave script:
...
The values for a and b that result can be seen below.

Performance: Expected output at 20kHz for a 1.65V amplitude signal
should be 1.167V.  Actual output is 1.05V
*/
// GREEN LED ON PB3
...
const float a[]={ 1.00000 , -2.36951 ,  2.31399 , -1.05467 ,  0.18738};
const float b[]={ 0.0048243 , 0.0192974 ,  0.0289461 ,  0.0192974 ,  0.0048243};
float ADCValue = 0;
int main()
{
    char buf[20];
    ...
    initUART(9600);
    initDAC();
    initADC(200000); // 200kHz sample rate
    enable_interrupts();
    displayRegisters();
    while(1)
    {                       
...
{
    // Execution speed using arm-none-eabi-gcc (GNU Tools for ARM Embedded Processors) 5.4.1 20160919 (release) [ARM/embedded-5-branch revision 240496]
    // Without optimization in command line: 19 microseconds
    // With -O1 : alternating between 2.44 and 5.2 microseconds
    // With -O2 : 2 microseconds
    // With -O3 : 3.96 microseconds 
    register float yn;
    register int index;
    float *xptr = x;
    float *yptr = y;
    ...
    *xptr = input;
    yn = (*xptr)*(*b);
    for (index = 1 ; index < FILTER_LENGTH; index++)
    {
        xptr++;
        yptr++;
        aptr++;
        bptr++;
        yn += (*xptr)*(*bptr) - (*yptr)*(*aptr);
        
    }
   
    y[0]=yn;    
     
    for (index = FILTER_LENGTH-1; index > 0; index--)
    {
        *xptr = *(xptr - 1);
        *yptr = *(yptr - 1);
        xptr--;
...
void ADC_ISR()
{
    ...
    ADCValue = (ADCValue) - 2048.0;
    writeDAC(filter((ADCValue+2048.0)));  // Write new output to DAC
    GPIOB_ODR &= ~BIT3;  // Turn off green led
}
void writeDAC(int Output)
{
    DAC_DHR12R1 = Output;
}
