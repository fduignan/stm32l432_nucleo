...
// Configure the I/O pins.  Will use PA2 as TX and PA15 as RX so setup for alternate function
	GPIOA_MODER |= (BIT31 | BIT5);
	GPIOA_MODER &= ~BIT30;
	GPIOA_MODER &= ~BIT4;
// Turn on pull-ups to suppress noise when idle
    GPIOA_PUPDR |= (BIT30 | BIT4);
    GPIOA_PUPDR &= ~BIT31;
    GPIOA_PUPDR &= ~BIT5;
...
// Enable the USART
        USART2_CR1 |= BIT0;
// Enable EXTIMR
        EXTI_IMR1 |= BIT27; // enable USART2 IRQ
        EXTI_EMR1 |= BIT27; // enable USART2 Event
...
int available()
{
    /// returns 1 if there are RX bytes available
...
int transmitting()
{
    // returns 1 if transmitter is busy
    if (USART2_CR1 & BIT3)
        return 1;
    else
        return 0;
